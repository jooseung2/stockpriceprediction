...
def label_then_save(df, interval):
    ...
    temp_df['label'] = temp_df.apply(lambda x: label(x['priceAfter{}'.format(interval)]),axis=1)
    file_name = 'labeledDf/{}.csv'.format(interval)
    temp_df[['article_title', 'label']].to_csv(file_name, sep=',', encoding='utf-8',index=False)
